...
def create_data_from_images(data_path, low_res_file_path, high_res_file_path, sigma, downsample_factor, levels):
  ...
  sample_image = convert_pgm_to_numpy(os.path.join(data_path, high_res_images[0]))
  ...
  for i in range(num_samples):
    high_res_image = convert_pgm_to_numpy(os.path.join(data_path, high_res_images[i]))
...
# print(im.shape)
# pyramid = generate_gaussian_pyramid(im, 2, 2, 5)
# plt.imshow(pyramid[4], cmap="gray")
# plt.show()
#create_data_from_images('data/CAFE-FACS-Orig')
#save_data('training_high')

IMAGE_PATH = 'data/CAFE-FACS-Orig'
